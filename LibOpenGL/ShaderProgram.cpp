...
GLuint ShaderProgram::getAtributeLocation(const char* atributeName,
    ...
    if(location < 0 && dieOnError) {
        ...
        NT_DIE(QString("%1: Attribute %2 not found!").arg(QString::fromStdString(m_ProgramName)).arg(atributeName));
#else
        NT_DIE(("%s: Attribute %s not found!\n", m_ProgramName.c_str(), atributeName));
#endif
    }
...
GLuint ShaderProgram::getUniformLocation(const char* uniformName,
    ...
    if(location < 0 && dieOnError) {
#ifdef NT_QT_FRAMEWORK
        NT_DIE(QString("%1: Uniform location %2 not found!").arg(QString::fromStdString(m_ProgramName)).arg(QString(uniformName)));
#else
        NT_DIE(("%s: Uniform location %s not found!\n", m_ProgramName.c_str(), uniformName));
#endif
    }
...
#ifdef NT_QT_FRAMEWORK
        NT_DIE(QString("%1: Uniform block index %2 not found!").arg(QString::fromStdString(m_ProgramName)).arg(QString(uniformBlockName)));
#else
...
void ShaderProgram::loadFile(String& fileContent, const char* fileName) {
    ...
    if(!file.is_open()) {
#ifdef NT_QT_FRAMEWORK
        NT_DIE(QString("%1: Cannot open file %2 for reading!").arg(QString::fromStdString(m_ProgramName)).arg(QString(fileName)));
#else
        NT_DIE(("%s: Cannot open file %s for reading!\n", m_ProgramName.c_str(), fileName));
#endif
    }
